<script>
  ...
  function toggle() {
    if (isLeaf())
      if (branch.selected == true) deselect();
      else select();

    if (branch.expanded) {
      if (canContract()) branch.expanded = false;
    } else branch.expanded = true;
  }
  ...
  const canContract = (i) =>
    selectedDecendents.size == 0 || !expandIfDecendantSelected;
